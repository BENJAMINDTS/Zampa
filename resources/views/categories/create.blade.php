        ...
        <form method="POST" action="{{ route('categories.store') }}" novalidate>
          ...
          <div class="mb-6">
            ...
            <select
              ...
              class="mt-1 block w-full h-10 rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:focus:border-indigo-600 dark:focus:ring-indigo-600 sm:text-sm @error('destination') border-red-500 dark:border-red-500 @enderror"
            >
              <option value="kitchen" @selected(old('destination') === 'kitchen')>Cocina — platos y postres</option>
              <option value="bar" @selected(old('destination') === 'bar')>Barra — bebidas y cócteles</option>
            </select>
            @error('destination')
